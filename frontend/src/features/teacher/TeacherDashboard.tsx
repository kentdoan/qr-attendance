...
import { CheckinRecord, QrToken, Session } from '../../types';
...
export default function TeacherDashboard() {
  ...
  async function handleCreate(e: FormEvent) {
    ...
    const res = await attendanceApi.createSession({
      ...
      title: title.trim() || 'Phiên điểm danh',
    });
    setCreating(false);
  ...
  async function handleExport() {
    if (!session) return;
    const res = await attendanceApi.getReport(session.id);
    if (res.success && res.data) exportReportToExcel(res.data);
  }
  ...
  const qrValue = qr ? JSON.stringify({ sessionId: qr.sessionId, token: qr.token }) : '';

  return (
    <DashboardLayout title="Giảng viên" subtitle="Tạo phiên & trình chiếu QR điểm danh động">
      {!session ? (
        <form onSubmit={handleCreate} className="mx-auto max-w-md rounded-2xl bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-lg font-semibold text-slate-800">Tạo phiên điểm danh</h2>
          {error && <div className="mb-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{error}</div>}
          <label className="mb-1 block text-sm font-medium text-slate-700">Tên buổi học / môn học</label>
          <input
            value={title}
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-semibold text-slate-800">{session.title}</h2>
              <span
                className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                  session.isOpen ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-slate-600'
                }`}
              >
              ...
              {qr && session.isOpen ? (
                <>
                  ...
                  <div className="mt-4 flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-slate-600">{secondsLeft}s</span>
                  </div>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-semibold text-slate-800">
                Đã điểm danh <span className="text-indigo-600">({records.length})</span>
              </h2>
              <button
                  ...
                  {records.length === 0 ? (
                    ...
                    records.map((r, i) => (
                      <tr key={r.id} className="border-t border-slate-100">
                        ...
                        <td className="px-2 py-2 font-medium text-slate-700">{r.studentName}</td>
